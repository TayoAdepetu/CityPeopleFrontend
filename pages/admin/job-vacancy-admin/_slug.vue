<template>
  ...
  <form @submit.prevent="editPost()" class="selectBank normalInput2 fullWidth form-control mt-2">         
      ...
      <div>
          ...
          <div class="form-group">
            <input type="text" v-model="selectedPost.title_slug" class="form-control" id="slug" required>
          </div>

          <div class="form-group">
            <input type="text" v-model="selectedPost.salary" class="form-control" id="slug" required>
          </div>

          <div class="form-group">
            <input type="text" v-model="selectedPost.location" class="form-control" id="slug" required>
          </div>

          <div class="form-group">
            <input type="text" v-model="selectedPost.function" class="form-control" id="slug" required>
          </div>
  ...
  export default { 
    ...
    {
        try {
          const { data } = await this.$axios.get(`/api/auth/jobs/${context.params.slug}`);
          this.jobs = data.data
          return true; 
        } catch (error) {
          this.loading = false;
    ...
    const { data } = await this.$axios.put(`/api/auth/update-post/${this.selectedPost.job_slug}`,
    {title: this.selectedPost.title,
      ...
      */
      ...
      async deleteJob()
      {
        await this.$axios.post(`/api/auth/delete-post/${this.selectedPost.job_slug}`);
        this.deleteJobModal = false;
        this.getJobs();
