<template>
   ...
   <!-- Comments List -->
   <div class="comments" v-if="comments" v-for="(comment,index) in commentsData" :key="comment.commentid">
       <!-- Comment -->
                   ...
                   <div class="comment-actions">
                       <ul class="list">
                           <li>Votes: {{comment.votes}}
                               <a class="upvotes" v-if="!comment.votedByUser" v-on:click="voteComment(comment.commentid,'directcomment',index,0,'up')">Likes</a>
                               <a class="downvotes" v-if="!comment.votedByUser" v-on:click="voteComment(comment.commentid,'directcomment',index,0,'down')">Dislikes</a>
                           </li>
                           <li class="spam">
                   ...
                   <div v-if="!spamCommentsReply[index2] || !replies.spam" class="comment reply">
                       ...
                       <div class="comment-box" style="background: grey;">
                           ...
                           <div class="comment-footer">
                               ...
                               <div class="comment-actions">
                                   <ul class="list">
                                       <li class="voteList">Total votes: {{replies.votes}}
...
export default {
   ...
   */
   ...
   methods: {
       fetchComments() {
           this.$axios.get('/api/comments/' + this.$route.params.slug).then(res => {
       ...
       voteComment(commentId, commentType, index, index2, voteType) {
           if (this.user) {
               this.$axios.post('/api/comments/' + commentId + '/vote', {
                   users_id: this.user.id,
                   vote: voteType
               }).then(res => {
                   if (res.data) {
       ...
       spamComment(commentId, commentType, index, index2) {
           ...
           if (this.user) {
               this.$axios.post('/api/comments/' + commentId + '/spam', {
                   users_id: this.user.id,
               }).then(res => {
                   if (commentType == 'directcomment') {
